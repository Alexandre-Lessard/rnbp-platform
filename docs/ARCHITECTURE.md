...
| **newsletter_subscribers** | Email addresses subscribed to the newsletter. Unique constraint on email. Carries the CASL trail: `consentSource`, `consentAt`, and `unsubscribedAt` — set once and never cleared by a later subscribe, so an opt-out cannot be silently undone. |
| **orders** | Stripe checkout orders. Tracks the Stripe session/payment intent IDs, total amount in cents, order status (`pending`, `paid`, `shipped`, `cancelled`), and shipping info. `userId` is nullable (guest checkout allowed, set null on user deletion). Also carries the campaign that produced the sale (`utmSource`, `utmMedium`, `utmCampaign`, `fbclid`), the advertising consent **frozen at checkout** (`adConsent`), and the deduplication id shared with the browser pixel (`capiEventId`, `capiSentAt`). Those five exist because the Stripe webhook arrives with no browser: no URL to read, and no consent to consult. |
...
| **ad_spend** | What a campaign cost, typed in by hand. `campaign` matches `orders.utmCampaign`, which is how the spend finds its conversions. Read the spend from Meta's API instead and you buy a system-user token to create, renew and guard, for a figure already on the invoice. |
