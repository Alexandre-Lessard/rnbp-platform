...
| **item_photos** | Photos attached to items, stored in Cloudflare R2. One photo per item can be marked `isPrimary`. The `GET /items` endpoint joins the first photo as `primaryPhotoUrl` for dashboard thumbnails. Cascades on item deletion (R2 files cleaned up). |
